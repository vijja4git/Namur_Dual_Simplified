...
/* µA -> ADC: inverse of namur_adc_to_current_ua */
...
static void test_global_fault_or(void)
{
    namur_system_state_t sys;
    namur_logic_init(&sys);
    sys.channel[0].fault = NAMUR_FAULT_LEAD_BREAK;
    assert(namur_logic_any_fault(&sys) == true);
    namur_logic_init(&sys);
    assert(namur_logic_any_fault(&sys) == false);
}
